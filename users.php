			...
			if ($act=='log') {
				...
				if (isset($session_old)) {
					if(mysqli_num_rows($session_search)){
						...
						if ($autolog==1) {
						setcookie("username",$user['login'],time()+(60*60*24*365),'/fox-club');
						setcookie("userpass",$user['pass'],time()+(60*60*24*365),'/fox-club');
						}else{
						setcookie("username",$user['login'],time()+(60*15),'/fox-club');
						setcookie("userpass",$user['pass'],time()+(60*15),'/fox-club');
						}
						$mysqli->query("DELETE FROM `session` WHERE `session_id` = '$session_old' ");
						$this->member = $user;
						$user_id=$user['id'];
						$user_name=$user['login'];
						setcookie("session_id",$session_new,time()+(60*15),'/fox-club');
						...
						setcookie("session_id",$sessionid,time()+(60*15),'/fox-club');
						setcookie("username",$username,time()+(60*15),'/fox-club');
						setcookie("userpass",$pass,time()+(60*15),'/fox-club');
						if (isset($_POST['check'])) {
						setcookie("autolog","1",time()+(60*60*24*365),'/fox-club');
						}
						$this->member= $user;
						}
						}
						...
						setcookie("username","",time()-3600,'/fox-club');
						setcookie("userpass","",time()-3600,'/fox-club');
						setcookie("session_id","",time()-3600,'/fox-club');
						header("location: index.php");
						exit;
						}
						...
						setcookie("userpass","",time()-3600,'/fox-club');
						setcookie("session_id","",time()-3600,'/fox-club');
						setcookie("autolog","",time()-3600,'/fox-club');
						header("location: index.php");
						exit;
						}
						}
